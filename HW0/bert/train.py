...
class Trainer:
        ...
        for epoch in range(self.args.epoch_num):
            ...
            for i, sample in enumerate(train_dataloader):
                ...
                if args.smooth_ratio > 0:
                    loss = criterion(outputs, smooth(targets, args.smooth_ratio))
                else:
    ...
    def run(self):
        ...
        warmup_scheduler = self.args.warmup_scheduler(optimizer, \
                            num_warmup_steps=int(self.args.warmup_ratio * len(train_dataloader) * self.args.epoch_num),
                            num_training_steps=len(train_dataloader) * self.args.epoch_num)
...
# Read hyperparameters from CMD
    ...
    parser.add_argument("--lr", default=1e-5, type=float, help="*e-5 are recommended")
